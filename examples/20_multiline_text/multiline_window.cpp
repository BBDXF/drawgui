...
void legend(std::ostream& out) {
  out << "  five panels, top to bottom: latin wrap, CJK wrap (no spaces - UAX#14\n"
      << "  is the only thing that can break this), mixed CJK+Latin+emoji, BiDi\n"
      << "  (Arabic embedded in Latin, reordered without mirroring the UI), and\n"
      << "  a max_lines=2 ellipsis truncation of the latin panel's own text.\n";
}
...
class Runner {
 ...
 private:
  ...
  void draw(multiline_scene::Scene& scene, dg::RasterSurface& surface);

  Settings settings_;
  std::ostream* out_;
  dg::WindowManager* manager_;
  dg::WindowId window_;
  std::optional<multiline_scene::Scene> scene_;
...
bool Runner::resize_if_needed() {
  ...
  if (scene_.has_value()) {
    ...
    scene_ = multiline_scene::build(spec_for(pixels), settings_.font_dir);
  }
  surface_ = dg::RasterSurface::create(pixels.width, pixels.height);
  return surface_.has_value() && scene_.has_value();
}

void Runner::draw(multiline_scene::Scene& scene, dg::RasterSurface& surface) {
  scene.tree.layout();
...
int Runner::run() {
  ...
  *out_ << "multiline text demo: a static scene - resize the window, nothing else moves.\n";
  legend(*out_);
...
std::optional<multiline_scene::Scene> rendered(const Settings& settings,
                                               std::optional<dg::RasterSurface>& surface) {
  surface = dg::RasterSurface::create(settings.size.width, settings.size.height);
  if (!surface.has_value()) {
    return std::nullopt;
  }
  multiline_scene::Scene scene =
      multiline_scene::build(spec_for(settings.size), settings.font_dir);
...
int run(const Settings& settings, std::ostream& out) {
  ...
  dg::WindowSpec spec;
  spec.title = "drawgui multiline text";
...
int dump_png(const Settings& settings, const std::string& path, std::ostream& out) {
  std::optional<dg::RasterSurface> surface;
  const std::optional<multiline_scene::Scene> scene = rendered(settings, surface);
