...
std::optional<Scene> build(const dg::TreeSpec& spec, const std::string& package_dir,
  ...
  dg::Expected<dg::ThemePackage, dg::ThemeLoadError> opened =
      dg::ThemePackage::open(package_dir);
  if (!opened) {
  ...
  if (bind_gap_to_token) {
    dg::PropWrite gap_write = dg::bind_token(tree, bindings, handles.row, DG_PROP_GAP, theme,
    ...
    (void)dg::set_prop(tree, handles.row, DG_PROP_GAP,
                       dg::PropValue::length(
                           static_cast<float>(theme.int_value(DG_TOKEN_SPACE_MD).value_or(8))));
  }
  ...
  tree.layout_full();
  return Scene{std::move(tree),  std::move(opened).value(),
               std::move(theme), std::move(bindings),
