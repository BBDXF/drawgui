// The complex-properties demo, on screen and headless.
//
//   (no arguments)              the demo window - resize it
//   --run-ms N                  close after N milliseconds
//   --verify-complex-properties the headless check. No display needed.
...
struct Options {
  ...
  cprops_window::Settings settings;
  std::string png_path;
};
...
std::optional<Options> parse_options(int argc, char** argv) {
  ...
  for (std::size_t i = 0; i < args.size(); ++i) {
    if (args[i] == "--verify-complex-properties") {
...
void usage() {
  std::cerr << "usage: drawgui_complex_properties [--run-ms N] [--dump-png FILE] "
...
int main(int argc, char** argv) {
  ...
  switch (options->mode) {
    case Mode::kVerify:
      return cprops_check::run(std::cout);
    case Mode::kDumpPng:
      return cprops_window::dump_png(options->settings, options->png_path, std::cout);
    case Mode::kWindow:
      break;
  }
  return cprops_window::run(options->settings, std::cout);
}
