...
void legend(std::ostream& out) {
  out << "  three panels, left to right: gradient, shadow, image - each built through\n"
      << "  the dedicated-setter channel (dg::set_gradient/set_shadow/set_image,\n"
      << "  design.md section 5.9.5) rather than by writing NodeStyle fields\n"
      << "  directly. The shadow panel's sliver past its own right/bottom edges IS\n"
      << "  the property painting outside its declared bounds - budgeted, hard-edged\n"
      << "  here for a byte-exact oracle (see doc/complex-properties.md).\n";
}

void log_transform(std::ostream& out, const dg::PropWrite& result) {
  out << "  dg::set_transform() (the channel's fourth, always-declined client): "
      << (result.status == dg::PropStatus::kUnsupported ? "kUnsupported" : "UNEXPECTED")
      << "\n    " << result.message << "\n";
}
...
class Runner {
 ...
 private:
  ...
  void draw(cprops_scene::Scene& scene, dg::RasterSurface& surface);

  Settings settings_;
  std::ostream* out_;
  dg::WindowManager* manager_;
  dg::WindowId window_;
  std::optional<cprops_scene::Scene> scene_;
...
bool Runner::resize_if_needed() {
  ...
  if (scene_.has_value()) {
    ...
    scene_ = cprops_scene::build(spec_for(pixels));
    log_transform(*out_, scene_->transform_result);
  }
  surface_ = dg::RasterSurface::create(pixels.width, pixels.height);
  return surface_.has_value() && scene_.has_value();
}

void Runner::draw(cprops_scene::Scene& scene, dg::RasterSurface& surface) {
  scene.tree.layout();
...
int Runner::run() {
  ...
  *out_ << "complex-properties demo: a static scene - resize the window, nothing else "
           "moves.\n";
  legend(*out_);
...
std::optional<cprops_scene::Scene> rendered(const Settings& settings,
  ...
  cprops_scene::Scene scene = cprops_scene::build(spec_for(settings.size));
  log_transform(out, scene.transform_result);
...
int run(const Settings& settings, std::ostream& out) {
  ...
  dg::WindowSpec spec;
  spec.title = "drawgui complex properties";
...
int dump_png(const Settings& settings, const std::string& path, std::ostream& out) {
  std::optional<dg::RasterSurface> surface;
  const std::optional<cprops_scene::Scene> scene = rendered(settings, surface, out);
