// Slice 6-1: the animation clock and implicit transitions, on screen.
//
//   (no arguments)        the demo window - resize it, hover the middle panel
//   --size WxH            open at a given size
//   --run-ms N            close after N milliseconds
//   --reduced-motion      honour design.md section 5.14.2's reduced-motion
//                         query (here, a plain flag standing in for the T2
//                         platform service this project does not query yet)
//   --idle-probe-ms N     block in WindowManager::pump() for N ms with
//                         nothing animating, and report the CPU time actually
//                         consumed - the measured answer to section 5.15.1's
//                         "wait_events() 阻塞，CPU 占用 0%"
//   --verify-animation    the headless check. No display needed.
//   --dump-png FILE       rasterize the scene's INITIAL state and write it out

#include <charconv>
#include <cstddef>
#include <cstdint>
#include <iostream>
#include <optional>
#include <string>
#include <system_error>
#include <vector>

#include "anim_check.h"
#include "anim_window.h"

namespace {

enum class Mode : std::uint8_t {
  kWindow,
  kVerify,
  kDumpPng,
  kIdleProbe,
};

struct Options {
  Mode mode = Mode::kWindow;
  anim_window::Settings settings;
  std::string png_path;
  int idle_probe_ms = 0;
};

std::optional<int> parse_int(const std::string& text, int minimum) {
  int value = 0;
  const char* end = text.data() + text.size();
  const std::from_chars_result parsed = std::from_chars(text.data(), end, value);
  if (parsed.ec != std::errc{} || parsed.ptr != end || value < minimum) {
    return std::nullopt;
  }
  return value;
}

bool parse_pair(const std::string& text, char separator, int minimum, int& first, int& second) {
  const std::size_t at = text.find(separator);
  if (at == std::string::npos) {
    return false;
  }
  const std::optional<int> left = parse_int(text.substr(0, at), minimum);
  const std::optional<int> right = parse_int(text.substr(at + 1), minimum);
  if (!left.has_value() || !right.has_value()) {
    return false;
  }
  first = *left;
  second = *right;
  return true;
}

bool apply_valued(const std::string& flag, const std::string& value, Options& options) {
  if (flag == "--size") {
    return parse_pair(value, 'x', 1, options.settings.size.width, options.settings.size.height);
  }
  if (flag == "--dump-png") {
    options.mode = Mode::kDumpPng;
    options.png_path = value;
    return true;
  }
  const std::optional<int> number = parse_int(value, 0);
  if (!number.has_value()) {
    return false;
  }
  if (flag == "--run-ms") {
    options.settings.run_ms = *number;
    return true;
  }
  if (flag == "--idle-probe-ms") {
    options.mode = Mode::kIdleProbe;
    options.idle_probe_ms = *number;
    return true;
  }
  return false;
}

std::optional<Options> parse_options(int argc, char** argv) {
  Options options;
  const std::vector<std::string> args{argv + 1, argv + argc};
  for (std::size_t i = 0; i < args.size(); ++i) {
    if (args[i] == "--verify-animation") {
      options.mode = Mode::kVerify;
      continue;
    }
    if (args[i] == "--reduced-motion") {
      options.settings.reduced_motion = true;
      continue;
    }
    if (i + 1 >= args.size() || !apply_valued(args[i], args[i + 1], options)) {
      return std::nullopt;
    }
    ++i;
  }
  return options;
}

void usage() {
  std::cerr << "usage: drawgui_animation [--size WxH] [--run-ms N] [--reduced-motion]\n"
            << "                         [--idle-probe-ms N] [--dump-png FILE]\n"
            << "                         [--verify-animation]\n";
}

}  // namespace

int main(int argc, char** argv) {
  const std::optional<Options> options = parse_options(argc, argv);
  if (!options.has_value()) {
    usage();
    return 2;
  }

  switch (options->mode) {
    case Mode::kVerify:
      return anim_check::run(std::cout);
    case Mode::kDumpPng:
      return anim_window::dump_png(options->settings, options->png_path, std::cout);
    case Mode::kIdleProbe:
      return anim_window::idle_probe(options->settings, options->idle_probe_ms, std::cout);
    case Mode::kWindow:
      break;
  }
  return anim_window::run(options->settings, std::cout);
}
